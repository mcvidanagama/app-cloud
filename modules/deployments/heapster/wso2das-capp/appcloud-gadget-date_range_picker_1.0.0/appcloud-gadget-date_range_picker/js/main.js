...
$(function() {
    ...
    var timeFrom = moment().subtract(30, 'minutes');
    var timeTo = moment();
    var message = {};
    ...
    if (timeUnit != null) {
        ...
        $("#date-select [role=date-update][data-value=Last30Minutes]").addClass("active");
    }
    ...
    $("#date-select [role=date-update]").click(function(){
        ...
        switch($(this).data('value')){
            case 'Last30Minutes':
                dateLabel.html(moment().subtract(30, 'minutes').format('MMMM D, YYYY hh:mm A') + ' - ' + moment().format('MMMM D, YYYY hh:mm A'));
                message = {
                    timeFrom: new Date(moment().subtract(30, 'minutes')).getTime(),
            ...
            case 'LastHour':
                ...
                break;
            case 'LastDay':
                dateLabel.html(moment().subtract(1, 'days').format('MMMM D, YYYY hh:mm A') + ' - ' + moment().format('MMMM D, YYYY hh:mm A'));
                message = {
                    timeFrom: new Date(moment().subtract(1, 'days')).getTime(),
                    timeTo: new Date(moment()).getTime(),
                    timeUnit: "Day"
                };
                gadgetUtil.updateURLParam("timeFrom", message.timeFrom.toString());
                ...
                break;
            case 'LastWeek':
                dateLabel.html(moment().subtract(7, 'days').format('MMMM D, YYYY hh:mm A') + ' - ' + moment().format('MMMM D, YYYY hh:mm A'));
                message = {
                    timeFrom: new Date(moment().subtract(7, 'days')).getTime(),
                    timeTo: new Date(moment()).getTime(),
                    timeUnit: "Day"
                };
                gadgetUtil.updateURLParam("timeFrom", message.timeFrom.toString());
